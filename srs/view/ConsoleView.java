...
public class ConsoleView implements View {
    ...
    public ConsoleView(Animals Animals) {
    ...
    public boolean showAddAnimalDialog() {
        ...
        while (true) {
            ...
            try (counter){
                ...
                counter.add();
                return Animals.createAnimal(parsedData.name, parsedData.birthDate, parsedData.animalGenius);
    ...
    public void showDetailInfoAnimalDialog() {
        String infoMessage = String.format("Введите номер животного (1 - %d)", Animals.getAnimals().size());
        System.out.println(infoMessage);
        while (true) {
            try {
                scanner = new Scanner(System.in);
                int animalNumber = scanner.nextInt();
                AbstractAnimal animal = Animals.getAnimals().get(animalNumber - 1);
    ...
    public int showRemoveAnimalDialog() {
        ...
        while (true) {
            try {
                ...
                return Animals.removeAnimal(animal);
            } catch (IndexOutOfBoundsException e) {
                System.out.println("Запись с таким номером отсутствует");
            }
        }
